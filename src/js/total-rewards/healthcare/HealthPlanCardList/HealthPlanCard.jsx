 ...
 */
const HealthPlanCard = ({ healthPlanName, healthPlanData, healthPlanType }) => {
  const benefitType = healthPlanName === 'Prescriptions' || healthPlanName === 'CVS Caremark' ? 'PRESCRIPTIONS' : healthPlanData.BENEFIT_TYPE;

  const cardClasses = `health-care__plan-coverage-card ${
    !benefitType ? 'health-care__plan-coverage-card--no-coverage' : ''
  } p-6 py-10 p-lg-2 p-xl-6 rounded`;

  const titleMarginClass =
    healthPlanData.PLAN_NAME &&
    (healthPlanData.PLAN_NAME.includes('High Performance HSA Plan') || healthPlanData.PLAN_NAME.includes('High Performance Copay Plan'))
      ? 'mt-0'
      : 'my-2';
  ...
  return (
    <div className={cardClasses}>
      ...
      <div className="health-care__plan-data p-4 rounded mb-3 mb-lg-6">
        <HealthPlanData benefitType={benefitType} benefitData={healthPlanData} benefitPlanType={healthPlanType} />
      </div>

      <div className="mb-2">
        <HealthPlanDescription benefitType={benefitType} benefitData={healthPlanData} />
      </div>
